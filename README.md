...
REST API на Spring Boot: учёт датчиков, инциденты (alerts), пользователи и роли, JWT в cookie, загрузка фото (Thymeleaf), PDF-отчёт при закрытии инцидента, уведомления в Telegram при **создании** инцидента и при **смене статуса** (`PUT .../status_change`).
